...
#pragma mark - Force in range
#define ENRANGE(_X_, _MIN_, _MAX_)  MAX(MIN(_X_, _MAX_), _MIN_)
#define ENRANGE_PT(_PT_, _RECT_) CGPointMake(ENRANGE(_PT_.x, _RECT_.origin.x, _RECT_.origin.x + _RECT_.size.width), ENRANGE(_PT_.y, _RECT_.origin.y, _RECT_.origin.y + _RECT_.size.height))
...
#define SAFE_PERFORM_MAIN(THE_OBJECT, THE_SELECTOR, THE_ARG) (([THE_OBJECT respondsToSelector:THE_SELECTOR]) ? [THE_OBJECT performSelectorOnMainThread:THE_SELECTOR withObject:THE_ARG waitUntilDone:NO] : nil)
